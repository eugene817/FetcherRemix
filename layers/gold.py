...
from layers.utils import _p, _s

MAX_MIN_SALARY = 18000
IRRELEVANT_KEYWORDS = r"""
(?i)(c\+\+|embedded|frontend|react|angular|migration|support|helpdesk|android|ios|flutter)
"""
SENIOR_KEYWORDS = r"""
(?i)(senior|lead|principal|architect|manager|head|director)
"""


async def generate_gold_report(
    filtered_postings: pl.DataFrame, number_of_rows: int
) -> None:
    if filtered_postings.is_empty():
        _s(
            "No new high-confidence matches found today. "
            "Skipping dashboard compilation."
        )
        return
    ...
    for _rank, job in enumerate(report_df.to_dicts(), 1):
        ...
        console.print(
            Panel(
                card_content,
                title=f"[bold green][{int(job['match_score'])}%]"
                f"[/bold green] {_rank}. {job['title']}",
    ...
    async with aiofiles.open(settings.gold_report_json, "w", encoding="utf-8") as f:
        await f.write(json.dumps(report_df.to_dicts(), ensure_ascii=False, indent=4))
